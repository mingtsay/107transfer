(function($,_I,_C,_T){$(function(){$.getJSON('transfer.min.json',function(transfer){var t=_I('transfer'),length=transfer.length
for(var i=0;i<length;++i){var school=transfer[i],space=_C('div'),container=_C('div'),school_seal=_C('img'),school_name=_T(school.name),school_published=_C('span'),school_title=_C('h3'),school_title_container=_C('div')
...
if(school.published){var table=_C('table'),table_body=_C('tbody'),price=_C('tr'),price_head=_C('th'),price_content=_C('td'),regist=_C('tr'),regist_head=_C('th'),regist_content=_C('td'),tests=_C('tr'),tests_head=_C('th'),tests_content=_C('td'),result=_C('tr'),result_head=_C('th'),result_content=_C('td'),signup=_C('tr'),signup_head=_C('th'),signup_content=_C('td')
...
for(var grade_id in school.grade){var grade=school.grade[grade_id],grade_table=_C('table'),grade_table_body=_C('tbody'),grade_name=_C('tr'),grade_name_head=_C('th'),grade_name_content=_C('td'),grade_date=_C('tr'),grade_date_head=_C('th'),grade_date_content=_C('td'),grade_qualify=_C('tr'),grade_qualify_head=_C('th'),grade_qualify_content=_C('td')
...
for(var j=0;j<grade.test.length;++j){var test_info=grade.test[j],test_row=_C('tr'),test_head=_C('th'),test_content=_C('td'),test_from=_T(test_info.time.from),test_to=_T(test_info.time.to),test_break=_C('br')
...
space.id=school.id
t.appendChild(space)
t.appendChild(container)}})})}(jQuery,(id)=>document.getElementById(id),(tag)=>document.createElement(tag),(text)=>document.createTextNode(text)))